...
# Questão 6
## Questão 6A
O parâmetro $\beta$ é dado por:
$$\hat{\beta} = \frac{\sum^{n}_{i=1} (x_{i} - \bar{x})y_{i}}{\sum^{n}_{i=1} (x_{i} - \bar{x})^{2}}$$
e a derivada é dada por:
$$\frac{dy}{dx} = \beta$$
e elasticidade:
$$\frac{dy}{dx} \frac{x}{y} = \beta \frac{x}{y}$$

## Questão 6B
$$log(y_{i}) = \alpha + \beta log(x_{i}) + u_{i}$$
$$E(log(y_{i})|X) = \alpha + \beta log(x_{i})$$
...
## Questão 7C
A hipótese de que $Var(u_{i}|x_{1i},...,x_{ki}) =  \sigma^{2}$ 
...
## Questão 8A
A hipótese de exogeneidade é necessária para que o estimador $\beta_{1}$ seja consistente?

## Questão 8B
Se $Var(u_{i}|x_{1i},x_{2i}) = x_{1i}\sigma^{2}$

## Questão 8C
Falso, existir correlação entre $x_{1i}$ e $x_{2i}$s
